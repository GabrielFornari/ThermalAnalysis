...
def imfeatures(folder, roi, imgIdxNames = []):
    ...
    if not imgIdxNames:
        ...
        imgFeatures = np.array([[-1.]*4 for i in range(nFiles)])
    else:
        imgFeatures = np.array([[-1.]*4 for i in range(len(imgIdxNames))])
...
################################
imgFeatures = imfeatures("D:/Flir E5x Data/Images/N14ZP7_noError_imgs/", cpu, [1, 2, 3, 4, 5])

# Frequency of shooting is 9 fps
#timeStamp = np.r_[0:1339] / 9.0
timeStamp = np.r_[0:5] / 9.0

plt.plot(timeStamp, imgFeatures[:, 2]/255*30 + 20)
plt.show()
